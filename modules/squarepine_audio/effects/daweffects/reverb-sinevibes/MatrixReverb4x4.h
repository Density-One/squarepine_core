...
class MatrixReverb4x4_Core
{
    
private:
    
    SimpleDelay mDelay[4];
...
public:
    ...
    void setSampleRate( float inSampleRate )
    {
        ...
        for( int i = 0; i < 4; i++ )
        {
            mDelay[i].setMaxDelayTime( delayTime[9] * 10.f );
    ...
    void processBlock( float *inBlock, float *inSize, float *inDecay, float *inModulation, float *inLowDamping, float *inHighDamping, float inDelayTimeOffset, int inInterpolation, float *outBlock, int blockSize )
    {
        ...
        for( int frame = 0; frame < blockSize; frame++ )
        {
            ...
            float time2 = delayTime[7] * timeScale + inDelayTimeOffset;
            float time3 = delayTime[9] * timeScale - inDelayTimeOffset;
            ...
            input[0] = inBlock[frame] + inDecay[frame] * ( feedback[0] + feedback[1] + feedback[2] + feedback[3] );
            input[1] = inBlock[frame] + inDecay[frame] * ( feedback[0] - feedback[1] + feedback[2] - feedback[3] );
            input[2] = inBlock[frame] + inDecay[frame] * ( feedback[0] + feedback[1] - feedback[2] - feedback[3] );
            input[3] = inBlock[frame] + inDecay[frame] * ( feedback[0] - feedback[1] - feedback[2] + feedback[3] );
...
class MatrixReverb4x4
{
    
private:
    
    MatrixReverb4x4_Core mMatrixReverbL;
    MatrixReverb4x4_Core mMatrixReverbR;
